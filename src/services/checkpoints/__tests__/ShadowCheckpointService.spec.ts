	...
	(klass, prefix) => {
		...
		describe(`${klass.name}#getContent and file rejection workflow`, () => {
			it("should delete newly created files when getContent throws 'does not exist' error", async () => {
				...
				// This tests the integration between ShadowCheckpointService and FilesChanged file rejection
				...
				// 4. Simulate FilesChanged file rejection: try to get content from baseHash (should throw)
				// This simulates what FilesChangedMessageHandler.revertFileToCheckpoint() does
				await expect(service.getContent(service.baseHash!, newFile)).rejects.toThrow(
					/does not exist|exists on disk, but not in/,
				)

				// 5. Since getContent threw an error, simulate the deletion logic from FilesChangedMessageHandler
				// In real FilesChanged, this would be handled by FilesChangedMessageHandler.revertFileToCheckpoint()
				try {
			...
			it("should restore file content when getContent succeeds for modified files", async () => {
				...
				// This tests the integration between ShadowCheckpointService and FilesChanged file rejection for existing files
				...
				// 4. Simulate FilesChanged file rejection: get original content from baseHash
				// This simulates what FilesChangedMessageHandler.revertFileToCheckpoint() does
				const previousContent = await service.getContent(service.baseHash!, testFile)
				expect(previousContent).toBe("Hello, world!")

				// 5. Simulate the restoration logic from FilesChangedMessageHandler
				// In real FilesChanged, this would be handled by FilesChangedMessageHandler.revertFileToCheckpoint()
			...
			it("should handle getContent with absolute vs relative paths correctly", async () => {
				...
				// This ensures FilesChangedMessageHandler path handling is compatible with ShadowCheckpointService
