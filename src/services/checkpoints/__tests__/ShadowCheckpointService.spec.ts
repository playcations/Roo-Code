...
describe.each([[RepoPerTaskCheckpointService, "RepoPerTaskCheckpointService"]])(
	...
	(klass, prefix) => {
		...
		describe(`${klass.name} excludes`, () => {
			it("writes recursive .git excludes and does not block on nested repos", async () => {
				// Set up a fresh workspace with a nested repo folder
				const shadowDir = path.join(tmpDir, `${prefix}-excludes-${Date.now()}`)
				const workspaceDir = path.join(tmpDir, `workspace-excludes-${Date.now()}`)
				await fs.mkdir(workspaceDir, { recursive: true })

				// Create a nested repo-like structure (.git with HEAD file)
				...
				await fs.writeFile(path.join(nestedGitDir, "HEAD"), "ref: refs/heads/main\n")

				// Initialize the service and ensure it does not throw
				const newService = await klass.create({ taskId, shadowDir, workspaceDir, log: () => {} })
				await expect(newService.initShadowGit()).resolves.not.toThrow()

				// Verify excludes include recursive .git pattern
				const excludesPath = path.join(newService.checkpointsDir, ".git", "info", "exclude")
				const excludes = (await fs.readFile(excludesPath, "utf8")).split("\n")
		...
		describe(`${klass.name} baseline initialization with FileChangeManager integration`, () => {
			...
			describe("Existing task scenario", () => {
				...
				it("should set baseline to fromHash when first checkpoint is created for existing task", async () => {
					...
					service.on("checkpoint", async (event) => {
