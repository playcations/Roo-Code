...
describe("FilesChangedOverview (Self-Managing)", () => {
	const mockExtensionState = {
		...
		// Ensure the experiment is enabled so the component renders
	...
	it("should handle checkpointRestored message", async () => {
		...
		// Component no longer requests changes; backend will push updates
		await waitFor(() => {
	...
	describe("Mid-Task FCO Enablement", () => {
		...
		it("should request fresh file changes when FCO is enabled mid-task", async () => {
			...
			// New behavior: no request is sent; backend is responsible for updates
			await waitFor(() => {
				expect(vscode.postMessage).not.toHaveBeenCalled()
			})
		})
	...
	describe("Layout and Display Integration", () => {
		...
		it("should render consistently when feature is enabled vs disabled", async () => {
			...
			expect(enabledState.experiments["filesChangedOverview"]).toBe(true)
			expect(disabledState.experiments["filesChangedOverview"]).toBe(false)
		})
		...
		describe("Virtualization and performance", () => {
			const makeFiles = (n: number) => ({
				baseCheckpoint: "base",
				files: Array.from({ length: n }).map((_, i) => ({
					uri: `src/file-${i}.ts`,
					type: "edit" as FileChangeType,
					fromCheckpoint: "base",
			...
			const getScrollContainer = () => {
				const root = screen.getByTestId("files-changed-overview")
				const el = root.querySelector('[class*="overflow-y-auto"]') as HTMLDivElement
				if (!el) throw new Error("Scroll container not found")
				return el
			}

			it("renders a slice and updates on scroll", async () => {
				renderComponent()
				simulateMessage({ type: "filesChanged", filesChanged: makeFiles(120) })
				const header = await screen.findByTestId("files-changed-header")
				fireEvent.click(header.closest('[role="button"]')!)

				const container = getScrollContainer()
				await waitFor(() => {
					const items = container.querySelectorAll('[data-testid^="file-item-"]')
					expect(items.length).toBeLessThanOrEqual(10)
					expect(screen.queryByTestId("file-item-src/file-0.ts")).toBeInTheDocument()
				})

				// Scroll down to reveal later items
				container.scrollTop = 60 * 20 // ITEM_HEIGHT * 20
				fireEvent.scroll(container)
				await waitFor(() => {
					expect(screen.queryByTestId("file-item-src/file-0.ts")).not.toBeInTheDocument()
					expect(screen.queryByTestId("file-item-src/file-20.ts")).toBeInTheDocument()
				})
			})

			it("handles very large lists without excessive DOM", async () => {
				renderComponent()
				simulateMessage({ type: "filesChanged", filesChanged: makeFiles(1000) })
				const header = await screen.findByTestId("files-changed-header")
				fireEvent.click(header.closest('[role="button"]')!)
			...
			it("does not virtualize at 50 but does at 51", async () => {
				// Exactly threshold: 50 -> no virtualization
				const { unmount } = renderComponent()
				simulateMessage({ type: "filesChanged", filesChanged: makeFiles(50) })
				const header50 = await screen.findByTestId("files-changed-header")
				fireEvent.click(header50.closest('[role="button"]')!)
				const container50 = getScrollContainer()
				await waitFor(() => {
					const items = container50.querySelectorAll('[data-testid^="file-item-"]')
					expect(items.length).toBe(50)
				})

				// Above threshold: 51 -> virtualization (<=10 rendered)
				unmount()
				renderComponent()
				simulateMessage({ type: "filesChanged", filesChanged: makeFiles(51) })
				const header51 = await screen.findByTestId("files-changed-header")
				fireEvent.click(header51.closest('[role="button"]')!)
				const container51 = getScrollContainer()
				await waitFor(() => {
					const items = container51.querySelectorAll('[data-testid^="file-item-"]')
					expect(items.length).toBeLessThanOrEqual(10)
				})
			})
		})
		...
		describe("Accessibility and keyboard", () => {
			it("exposes ARIA attributes and toggles via keyboard", async () => {
				renderComponent()
				simulateMessage({ type: "filesChanged", filesChanged: mockChangeset })
				await waitFor(() => {
					expect(screen.getByTestId("files-changed-overview")).toBeInTheDocument()
				})

				const button = (await screen.findByTestId("files-changed-header")).closest(
					'[role="button"]',
				) as HTMLElement
				expect(button).toHaveAttribute("aria-expanded", "false")
				expect(button).toHaveAttribute("tabindex", "0")
				expect(button.getAttribute("aria-label")).toMatch(/2 files collapsed/)

				// Enter toggles
				fireEvent.keyDown(button, { key: "Enter" })
				await waitFor(() => {
					expect(button).toHaveAttribute("aria-expanded", "true")
					expect(button.getAttribute("aria-label")).toMatch(/2 files expanded/)
				})
		...
		// ===== DEBOUNCE =====
		describe("Debounced actions", () => {
			it("triggers Accept All action", async () => {
				renderComponent()
				simulateMessage({ type: "filesChanged", filesChanged: mockChangeset })
				await waitFor(() => {
					expect(screen.getByTestId("files-changed-overview")).toBeInTheDocument()
				})
				const header = (await screen.findByTestId("files-changed-header")).closest('[role="button"]')!
				fireEvent.click(header)
				const btn = await screen.findByTestId("accept-all-button")
				btn.click()
				expect(vscode.postMessage).toHaveBeenCalled()
				expect(vscode.postMessage).toHaveBeenCalledWith({ type: "acceptAllFileChanges" })
			})

			it("triggers Reject All with URIs", async () => {
				renderComponent()
				simulateMessage({ type: "filesChanged", filesChanged: mockChangeset })
				await waitFor(() => {
					expect(screen.getByTestId("files-changed-overview")).toBeInTheDocument()
				...
				const header = (await screen.findByTestId("files-changed-header")).closest('[role="button"]')!
				fireEvent.click(header)
				const btn = await screen.findByTestId("reject-all-button")
				btn.click()
				expect(vscode.postMessage).toHaveBeenCalled()
				expect(vscode.postMessage).toHaveBeenCalledWith({
					type: "rejectAllFileChanges",
					uris: mockChangeset.files.map((f) => f.uri),
				})
			})
		})

		it("disables buttons while processing operations", async () => {
			renderComponent()
			simulateMessage({ type: "filesChanged", filesChanged: mockChangeset })
			await waitFor(() => {
				expect(screen.getByTestId("files-changed-overview")).toBeInTheDocument()
			...
			const header = (await screen.findByTestId("files-changed-header")).closest('[role="button"]')!
			fireEvent.click(header)
			const acceptBtn = await screen.findByTestId("accept-all-button")
			// Trigger processing state via debounced handler
			acceptBtn.click()
			await waitFor(() => {
				expect(screen.getByTestId("accept-all-button")).toBeDisabled()
				expect(screen.getByTestId("reject-all-button")).toBeDisabled()
			})
		})

		// ===== MESSAGE ROBUSTNESS / REGRESSION =====
		describe("Message robustness and regressions", () => {
			it("ignores malformed messages", async () => {
				renderComponent()
				simulateMessage({ type: "filesChanged", filesChanged: mockChangeset })
				const overview = await screen.findByTestId("files-changed-overview")
				// Malformed/unknown
				window.dispatchEvent(new MessageEvent("message", { data: null as any }))
				window.dispatchEvent(new MessageEvent("message", { data: {} as any }))
				window.dispatchEvent(new MessageEvent("message", { data: { type: "unknown" } as any }))
				// Should remain intact
				expect(overview).toBeInTheDocument()
			})

			it("does not send request on checkpoint event", async () => {
				vi.clearAllMocks()
				renderComponent()
				simulateMessage({ type: "checkpoint", checkpoint: "abc", previousCheckpoint: "def" })
				await new Promise((r) => setTimeout(r, 0))
				expect(vscode.postMessage).not.toHaveBeenCalled()
			})

			it("clears when filesChanged is null", async () => {
				renderComponent()
				// Seed
				simulateMessage({ type: "filesChanged", filesChanged: mockChangeset })
				await waitFor(() => {
				...
				// Null payload clears
				simulateMessage({ type: "filesChanged", filesChanged: null as any })
				await waitFor(() => {
					expect(screen.queryByTestId("files-changed-overview")).not.toBeInTheDocument()
				})
			})
		...
		describe("Theme sanity", () => {
			...
			it("renders consistently under light theme", async () => {
				applyTheme({
			...
			it("renders consistently under dark theme", async () => {
				applyTheme({
