...
describe("FilesChangedOverview (Self-Managing)", () => {
	const mockExtensionState = {
		experiments: { [EXPERIMENT_IDS.FILES_CHANGED_OVERVIEW]: true },
	...
	it("should not render when filesChangedEnabled is false", () => {
		const disabledState = {
			...mockExtensionState,
			experiments: { [EXPERIMENT_IDS.FILES_CHANGED_OVERVIEW]: false },
		}
	...
	describe("Layout and Display Integration", () => {
		...
		it("should render consistently when feature is enabled vs disabled", async () => {
			...
			expect(enabledState.experiments?.[EXPERIMENT_IDS.FILES_CHANGED_OVERVIEW]).toBe(true)
			expect(disabledState.experiments?.[EXPERIMENT_IDS.FILES_CHANGED_OVERVIEW]).toBe(false)
		})
